...
void ABatteryCollectorCharacter::CollectPickups()
{
	...
	// foreach actor
	for (int32 iCollected = 0; iCollected < OverlappingActors.Num(); iCollected++)
	{
		// cast to pickup
		APickup* const TestPickup = Cast<APickup>(OverlappingActors[iCollected]);

		// if cast was successful
		if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive())
		{
			// call the wasCollected function
